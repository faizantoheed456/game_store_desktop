...
class ImageManager:
    ...
    def get_image(url, filename, size=(300, 170), callback=None):
        ...
        ImageManager._executor.submit(ImageManager._load_task, url, filepath, filename, size, callback)
        return None # Return None while loading

    @staticmethod
    def _load_task(url, filepath, filename, size, callback):
        try:
            # Check Local Disk
            if os.path.exists(filepath):
                ImageManager._load_and_cache(filepath, filename, size, callback)
                return

            # Download if not on disk
            if url:
                ImageManager._async_download(url, filepath, filename, size, callback)
        except Exception:
            pass

    @staticmethod
    def _load_and_cache(filepath, filename, size, callback=None):
        try:
